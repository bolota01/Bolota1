...
-- Script starts here
local helicam = false
local supervolito2_hash = GetHashKey("supervolito2")
local fov = (fov_max+fov_min)*0.5
local vision_state = 0 -- 0 is normal, 1 is nightmode, 2 is thermal vision
Citizen.CreateThread(function()
	while true do
        Citizen.Wait(0)
		if IsPlayerInSupervolito2() then
...
function IsPlayerInSupervolito2()
	local lPed = GetPlayerPed(-1)
	local vehicle = GetVehiclePedIsIn(lPed)
	return IsVehicleModel(vehicle, supervolito2_hash)
end

...
local radar =
{
	shown = false,
	freeze = false,
	info = "~y~Iniciando Radar...... ",
	info2 = "~y~Iniciando Radar...... ",
	minSpeed = 5.0,
	maxSpeed = 75.0,
}
...
Citizen.CreateThread( function()
	...
	while true do
		...
		if IsControlJustPressed(1, 128) and IsPedInAnyPoliceVehicle(GetPlayerPed(-1)) then
			...
			if radar.shown then 
				radar.shown = false 
				radar.info = string.format("~y~Iniciando Radar...... ")
				radar.info2 = string.format("~y~Iniciando Radar...... ")
			else 
				radar.shown = true 
			end		
                Wait(75)
		...
		if radar.shown  then
			...
			DrawAdvancedText(0.591, 0.953, 0.005, 0.0028, 0.4, "Radar Traseiro", 0, 191, 255, 255, 6, 0)

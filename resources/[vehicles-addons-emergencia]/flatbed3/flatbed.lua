...
local attachmentOffset = {vector3(0.0, 1.5, 0.2), vector3(0.0, 0.0, 0.0)}

local bedController = {vector3(-2.5, -3.8, -1.3), vector3(0.0, 0.0, 0.0)}
...
AddTextEntry(controllerMessageLoweredCar, "Pressione ~INPUT_CONTEXT~ para ~y~levantar ~w~a prancha.~n~Pressione ~INPUT_DETONATE~ para ~r~soltar ~w~o veículo.")
AddTextEntry(controllerMessageLoweredNoCar, "Pressione ~INPUT_CONTEXT~ para ~y~levantar ~w~a prancha.~n~Pressione ~INPUT_DETONATE~ para ~g~fixar ~w~o veículo.")
AddTextEntry(controllerMessageRaised, "Pressione ~INPUT_CONTEXT~ para ~y~baixar ~w~a prancha.")
...
function drawMarker(pos)
    ...
    if dist < 5.0 then
        DrawMarker(1, pos, vector3(0.0, 0.0, 0.0), vector3(0.0, 0.0, 0.0), vector3(1.0, 1.0, 0.5), 204, 125, 0, 200)
        if dist < 1.5 then
            return true
        end
    end

...
function LoadBlips()
  for k,v in ipairs(cinemaLocations) do
    ...
    AddTextComponentSubstringPlayerName("Cinemas")
...
-- Check if the cinema is open or closed.
          if hour < openingHour or hour > closingHour then
			helpDisplay("O filme está em cartaz entre 1am e 22pm.", 0)
          else
			helpDisplay("Pressione ~INPUT_CONTEXT~ para entrar no cinema.", 0)
...
AddEventHandler("vrp_cinema:movie", function (data)
	...
	AddTextComponentString("Pressione ~ r ~ E ~ w ~ para sair do cinema.")

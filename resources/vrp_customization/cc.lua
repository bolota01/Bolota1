local language = {
	...
	featuresList[1]={name=language.eyeColor,items = eyeColors,index= 1}
	...
	function updateCharacter()
		...
		SetPedEyeColor(GetPlayerPed(-1),featuresList[1].index-1)
	...
	AddEventHandler('loadCharacter', function(superString)
		...
		for i=6, 26 do
			featuresList[i-5].index=tonumber(charData[i])
		end
...
Citizen.CreateThread(function()
    ...
    while true do
		...
		elseif WarMenu.IsMenuOpened('menuFeatures') then
			if WarMenu.ComboBox(featuresList[1].name, featuresList[1].items, featuresList[1].index, featuresList[1].index, function(currentIndex, selectedIndex)
				if featuresList[1].index~=currentIndex then
					IsBlendFace=true
				end
				featuresList[1].index = currentIndex
			end) then
		end
			for i=2,20 do

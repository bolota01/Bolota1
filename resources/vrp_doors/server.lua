  ...
  if vRP.hasPermission({user_id, "#"..cfg.list[id].key..".>0"}) or vRP.hasPermission({user_id,cfg.list[id].permission}) then
    ...
    SetTimeout(1000, function()
      ...
      if cfg.list[id].locked then
        vRPclient.notify(player, {"A porta está trancada "..cfg.list[id].name})
      else
        vRPclient.notify(player, {"Porta destrancada "..cfg.list[id].name})
      end
    end)
  else
    vRPclient.notify(player, {"Chave da porta ausente "..cfg.list[id].name})
  end
end)
